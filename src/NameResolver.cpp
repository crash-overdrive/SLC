...
Resolver::findField(const std::vector<std::string> &name) const {
  ...
  if (!staticMethod || !isStaticVisible(matchDecl, staticMethod)) {
    return std::nullopt;
  }
...
Resolver::findField(Env::Type type, const std::string &identifier) const {
  ...
  if (!field || !isVisible(matchDecl, field)) {
    return std::nullopt;
  }
  return field->type;
}
...
Resolver::findMethod(const std::vector<std::string> &name,
  ...
  if (it == name.end() - 1) {
    ...
    if (!staticField || !isStaticVisible(matchDecl, staticField)) {
      return std::nullopt;
    }
...
Resolver::findMethod(Env::Type type, const std::string &identifier,
  ...
  if (!method || !isVisible(type.declare, method)) {
    return std::nullopt;
  }
...
template <class T>
bool Resolver::isStaticVisible(const Env::TypeDeclaration *other, T t) const {
  return isVisible(other, t) &&
         t->modifiers.find(Env::Modifier::Static) != t->modifiers.end();
}
