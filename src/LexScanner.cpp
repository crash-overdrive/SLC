...
bool Lex::Scanner::maximalMunch(std::deque<char> input) {
  ...
  while (!input.empty()) {
    ...
    Lex::AcceptingStateInfo acceptingStateInfo = finalDfa.getAcceptingState(currentState);
  ...
  if (!processedTransitionSymbols.empty()) {
    ...
    std::cout << "Scanning failed at end of file, no matched pattern for " << garbage << std::endl;
    return false;
  }
