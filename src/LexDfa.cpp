...
Lex::AcceptingStateInfo::AcceptingStateInfo(std::string tokenKind, int tokenPriority) :
  tokenKind(tokenKind), tokenPriority(tokenPriority) {};


// bool Lex::AcceptingStateInfo::operator==(const AcceptingStateInfo& acceptingState) const {
...
void Lex::Dfa::addTransition(int previousState, char transitionSymbol, int nextState) {
  ...
  transitions.find(previousState)->second.find(transitionSymbol) == transitions.find(previousState)->second.end());

  transitions[previousState][transitionSymbol] = nextState;
}


void Lex::Dfa::addAcceptingState(int state, Lex::AcceptingStateInfo newAcceptingStateInfo) {
  assert(states.find(state) != states.end());
...
void Lex::Dfa::printInfo() {
  ...
  for (auto const& acceptingState: acceptingStates) {
    std::cout << "State: " << acceptingState.first << " Token: " << acceptingState.second.tokenKind << " Priority: " << acceptingState.second.tokenPriority << std::endl;
  }

  std::cout << "Dfa transitions" << std::endl;
  for (auto const& transition: transitions) {
    for (auto const& transitionInfo : transition.second) {
      std::cout << "Previous State: " << transition.first << " Transition character: " << transitionInfo.first << " Next State: " << transitionInfo.second << std::endl;
    }
  }
}
