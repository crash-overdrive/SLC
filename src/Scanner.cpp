...
bool Lex::Dfa::transitionForward(char symbol) {
  ...
  // transition in DFA corresponding to symbol
  for (auto const& transition: transitions) {
    if(transition.previousStates == currentStates && transition.transitionSymbol == symbol) {
      currentStates = transition.nextStates;
      std::cout << "Transitioned.." << std::endl;
      break;
    }
  }

  // current state in accepting state
  for (auto const& acceptingState : acceptingStates) {
    if (currentStates == acceptingState.dfaStates) {
      std::string newAcceptedLexeme(processedTransitionSymbols.begin(), processedTransitionSymbols.end());

      processedTransitionSymbols.clear();

      lastKnownAcceptingToken.lexeme += newAcceptedLexeme;
      lastKnownAcceptingToken.kind = acceptingState.tokenKind;

      break;
    }
  }

  if (currentStates == trapState) {
    return false;
  }
  return true;
}

Lex::Token Lex::Dfa::getLastKnownAcceptingToken() {
  if (lastKnownAcceptingToken.kind == "UNINITIALISED_TOKEN") {
    // TODO: make scan fail if this happens
    std::cout << "VERY VERY BAD";
  }

  Token token(lastKnownAcceptingToken.kind, lastKnownAcceptingToken.lexeme);
  lastKnownAcceptingToken = Token();
  return token;
}

void Lex::Dfa::resetDfa() {
  currentStates = startStates;
}

std::vector<Lex::Token> Lex::Dfa::maximalMunch() {
  char ch;
  std::deque<char> input;
  std::vector<Lex::Token> tokensGenerated;

  // get all the input from the cin stream
  while (std::cin >> ch) {
    input.push_back(ch);
  }
  ...
  while (!input.empty()) {
    ...
    if (transitionForward(ch) == false) {
      tokensGenerated.push_back(getLastKnownAcceptingToken());
      resetDfa();
  ...
  if (!processedTransitionSymbols.empty()) {
    std::cout << "Got some garbage left over at the end.." << std::endl;
  }
...
std::vector<int> Lex::Nfa::epsilonClosure(std::vector<int> givenStates) {
  for (auto const &state : givenStates) {
    if (std::find(states.begin(), states.end(), state) == states.end()) {
      std::cout << "Error computing Epsilon closure for state: " << state
                << " .. State doesnt exist in Nfa" << std::endl;
      return {};
    }
  }
  ...
  while (!workList.empty()) {
    ...
    for (auto const &transition : transitions) {
      if (transition.transitionSymbol == EPSILON &&
          transition.previousState == currentState) {
        int newState = transition.nextState;
        if (std::find(epsilonClosureList.begin(), epsilonClosureList.end(),
                      newState) == epsilonClosureList.end()) {
      ...
      for (auto const &state : currentStates) {
        for (auto const &transition : transitions) {
          if (transition.previousState == state &&
              transition.transitionSymbol == alphabet) {
            // std::cout << "State: " << state << " Alphabet: " << int{alphabet}
            // << " State added: " << transition.nextState << std::endl;
...
Lex::Dfa Lex::Nfa::convertToDfa() {
  ...
  convertedDfa.currentStates = convertedDfa.startStates;
  return convertedDfa;
}
...
Lex::Nfa Lex::concatenate(Nfa& firstNfa, Nfa& secondNfa) {
  ...
  for (auto const &firstNfaTransition : firstNfa.transitions) {
    finalNfa.addTransition(firstNfaTransition.previousState,
                           firstNfaTransition.nextState,
                           firstNfaTransition.transitionSymbol);
  }

  // add second Nfa transitions
  for (auto const &secondNfaTransition : secondNfa.transitions) {
    finalNfa.addTransition(secondNfaTransition.previousState,
                           secondNfaTransition.nextState,
                           secondNfaTransition.transitionSymbol);
  }
...
Lex::Nfa Lex::orSelection(Nfa& firstNfa, Nfa& secondNfa) {
  ...
  int auxillaryState =
      firstNfa.getNumberOfStates() + secondNfa.getNumberOfStates() + 1;
  ...
  // add epsilon transition from auxillary state : stateSize1 + stateSize2 + 1
  // to starting state of first and second Nfa
...
Lex::Nfa Lex::kleeneStar(Nfa& nfa) {
  ...
  for (auto const &nfaTransition : nfa.transitions) {
    finalNfa.addTransition(nfaTransition.previousState, nfaTransition.nextState,
                           nfaTransition.transitionSymbol);
  }
  ...
  finalNfa.setAcceptingStates({NfaAcceptingState(auxillaryState, "UNINITIALISED", INT32_MAX)});
...
int main(int argc, char *argv[]) {
  ...
  (void)argv;
  // if (argc != 2) {
  // std::cout << "Invalid number of arguments passed, required: 2, got: " <<
  // argc << std::endl; return 42;
  //}
  ...
  // definitions.push_back(Lex::Definition(line.substr(0, position),
  // line.substr(position + delimiter.length())));
  //}
  // file.close();
  //} else {
  ...
  // std::cout << "[" << definition.type << "] {" << definition.regex << "}" <<
  // std::endl;
  //}
  ...
  std::vector<Lex::Token> tokens = dfa.maximalMunch();
