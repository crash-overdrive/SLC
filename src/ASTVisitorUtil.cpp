...
void PropertiesVisitor::visit(const PrimitiveType &primitiveType) {
  variableDescriptor = Env::VariableDescriptor{Env::VariableType::SimpleType, {primitiveType.getType()}};
}

void PropertiesVisitor::visit(const VoidType &voidType) {
  variableDescriptor = Env::VariableDescriptor{Env::VariableType::SimpleType, {voidType.getType()}};
}

void PropertiesVisitor::visit(const SimpleType &simpleType) {
  NameVisitor nameVisitor;
  nameVisitor.dispatchChildren(simpleType);
  variableDescriptor = Env::VariableDescriptor{Env::VariableType::SimpleType, nameVisitor.getName()};
}

void PropertiesVisitor::visit(const ArrayType &arrayType) {
  NameVisitor nameVisitor;
  nameVisitor.dispatchChildren(arrayType);
  variableDescriptor = Env::VariableDescriptor{Env::VariableType::ArrayType, nameVisitor.getName()};
}
...
std::string PropertiesVisitor::getIdentifier() {
